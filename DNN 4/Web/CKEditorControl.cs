    ...
    public class CKEditorControl : WebControl, IPostBackDataHandler
    {
            ...
            Settings
        {
            get
            {
                ...
                // fix oEmbed/oembed issue and other bad settings// fix oEmbed/oembed issue and other bad settings
                    ...
                    case Browser.StandardBrowser:
                        {
                            ...
                            DataCache.SetCache("CKDNNIsAuthorized", CKDNNIsAuthorized, true);
                        }
        ...
        private static string SResXFile
        {
            get
            {
                return string.Format("Providers/HtmlEditorProviders/CKEditor/{0}/Options.aspx.resx", Localization.LocalResourceDirectory);
            }
        }
            ...
            try
            {
                ...
                if (currentValue == null | !postedValue.Equals(currentValue))
                {
                    ...
                    else
                    {
                        ...
                        // Add Latex Plugin
                        if (postedValue.Contains("<span class=\"math-text\">") && !postedValue.Contains("shCore.js"))
                        {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            editorScript.AppendFormat(
                @"if (CKEDITOR && CKEDITOR.instances && CKEDITOR.instances.{0}) {{ CKEDITOR.instances.{0}.updateElement();}}",
                editorFixedId);
        ...
        protected override void Render(HtmlTextWriter outWriter)
        {
            ...
            if (PortalSecurity.IsInRoles(this._portalSettings.AdministratorRoleName))
            {
                outWriter.Write(
                    ...
                    Localization.GetString(
                        "Options.Text", SResXFile, Globals.GetPortalSettings(), CultureInfo.CurrentCulture.Name, true));
            ...
            outWriter.Write(
                ...
                Localization.GetString(
                    "Refresh.Text", SResXFile, Globals.GetPortalSettings(), CultureInfo.CurrentCulture.Name, true));
        ...
        private void CKEditorInit(object sender, EventArgs e)
        {
            ...
            if (HttpContext.Current.Items["jquery_registered"] == null)
            {
                cs.RegisterClientScriptInclude(
                    csType, "jquery_registered", "//ajax.googleapis.com/ajax/libs/jquery/1/jquery.min.js");
            ...
            if (isUserAccountsModule)
            {
                if (File.Exists(this.Context.Server.MapPath("~/Providers/HtmlEditorProviders/CKEditor/ckeditor.js")))
                {
                    ScriptManager.RegisterClientScriptInclude(
                        this, csType, CsName, Globals.ResolveUrl("~/Providers/HtmlEditorProviders/CKEditor/ckeditor.js"));
                }
        ...
        private string SetUserToolbar(string alternateConfigSubFolder)
        {
            ...
            foreach (var roleToolbar in this.currentSettings.ToolBarRoles)
            {
                ...
                ToolbarRoles toolbar1 = roleToolbar;
                if (!listToolbarSets.Any(toolbarSel => toolbarSel.Name.Equals(toolbar1.Toolbar)))
                {
                    continue;
                }
