...
namespace WatchersNET.CKEditor.Utilities
{
    ...
    public class ToolbarUtil
    {
        ...
        /// </summary>
        /// <typeparam name="A">type parameter A</typeparam>
        /// <typeparam name="B">type parameter B</typeparam>
        /// <param name="prevResult">The previous result.</param>
        /// <param name="currentElement">The current element.</param>
        /// <returns></returns>
        public delegate A AggregateAction<A, in B>(A prevResult, B currentElement);
        ...
        public static Tagg Aggregate<Tcoll, Tagg>(
            IEnumerable<Tcoll> source,
            Tagg seed,
            AggregateAction<Tagg, Tcoll> func)
        {
        ...
        public static List<ToolbarButton> LoadToolBarButtons(string homeDirPath)
        {
            ...
            using (
                TextReader textReader =
                    new StreamReader(
                        new FileStream(
                            Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarButtonXmlFileName),
                            FileMode.Open,
                            FileAccess.Read,
                            FileShare.Read)))
            {
        ...
        public static void CreateDefaultToolbarButtonXml(string savePath)
        {
            ...
            using (
                var textWriter =
                    new StreamWriter(Path.Combine(savePath, Constants.SettingConstants.ToolbarButtonXmlFileName)))
            {
        ...
        public static void CreateDefaultToolbarSetXml(string savePath)
        {
            ...
            using (
                var textWriter =
                    new StreamWriter(Path.Combine(savePath, Constants.SettingConstants.ToolbarSetXmlFileName)))
            {
        ...
        {
            return Aggregate(source, 0, (prev, curr) => curr > prev ? curr : prev);
        }
        ...
        {
            return Aggregate(source, 0, (prev, curr) => curr + prev);
        }
        ...
        internal static List<ToolbarSet> GetToolbars(string homeDirPath, string alternateConfigSubFolder)
        {
            ...
            if (File.Exists(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarXmlFileName)))
            {
                ImportOldToolbarXml(homeDirPath);
            }

            if (!File.Exists(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarSetXmlFileName)))
            {
                if (!File.Exists(Path.Combine(Globals.HostMapPath, Constants.SettingConstants.ToolbarSetXmlFileName)))
                {
                ...
                File.Copy(
                    Path.Combine(Globals.HostMapPath, Constants.SettingConstants.ToolbarSetXmlFileName),
                    Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarSetXmlFileName));
            }

            // also create default CKToolbarButtons.xml if needed
            if (!File.Exists(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarButtonXmlFileName)))
            {
                if (!File.Exists(Path.Combine(Globals.HostMapPath, Constants.SettingConstants.ToolbarButtonXmlFileName)))
                {
                    CreateDefaultToolbarButtonXml(Globals.HostMapPath);
                }

                File.Copy(
                    Path.Combine(Globals.HostMapPath, Constants.SettingConstants.ToolbarButtonXmlFileName),
                    Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarButtonXmlFileName));
            }
            ...
            catch (Exception)
            {
                if (!createDefault)
                {
                    ...
                    if (File.Exists(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarSetXmlFileName)))
                    {
                        File.Delete(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarSetXmlFileName));
                    }
        ...
        internal static void ImportOldToolbarXml(string homeDirPath)
        {
            ...
            if (File.Exists(Path.Combine(Globals.HostMapPath, Constants.SettingConstants.ToolbarXmlFileName)))
            {
                File.Delete(Path.Combine(Globals.HostMapPath, Constants.SettingConstants.ToolbarXmlFileName));
            }

            // Import old xml file
            var oldXmlPath = Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarXmlFileName);
            ...
            using (
                var textWriter =
                    new StreamWriter(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarSetXmlFileName)))
            {
            ...
            // Delete old xml filde
            File.Delete(Path.Combine(homeDirPath, Constants.SettingConstants.ToolbarXmlFileName));
        }
