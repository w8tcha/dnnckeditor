...
namespace WatchersNET.CKEditor.Browser
{
    ...
    public class ProcessImage : IHttpHandler
    {
        ...
        private static string GenerateName(string sNewFileName, string sSourceFullPath)
        {
            ...
            if (!string.IsNullOrEmpty(sNewFileName))
            {
                sNewFilePath = Path.Combine(sSourcePath, sNewFileName + sExtension);
            }
            else
            {
                sNewFilePath = Path.Combine(
                    sSourcePath, 
                    string.Format("{0}_crop{1}", Path.GetFileNameWithoutExtension(sSourceFullPath), sExtension));
            }
