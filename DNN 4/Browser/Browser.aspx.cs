...
namespace WatchersNET.CKEditor.Browser
{
    ...
    public partial class Browser : Page
    {
        ...
        public DataTable GetFiles(FolderInfo currentFolderInfo)
        {
            ...
            var files =
                    filesArrayList.OfType<DotNetNuke.Services.FileSystem.FileInfo>().ToList();
        ...
        protected void CmdCloseClick(object sender, EventArgs e)
        {
            ...
            {
                if (this.treeTabs.SelectedNode != null)
                {
                    var tabController = new TabController();

                    var selectTab = tabController.GetTab(
                        int.Parse(this.treeTabs.SelectedValue), this._portalSettings.PortalId, true);
        ...
        {
            fileUrl = !string.IsNullOrEmpty(fileUrl)
                           ? string.Format(!fileUrl.EndsWith("/") ? "{0}/{1}" : "{0}{1}", fileUrl, fileName)
            ...
            // string _CKEditorName = request.QueryString["CKEditor"];
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.extensionWhiteList = Convert.ToString(Globals.HostSettings["FileExtensions"]);
        ...
        /// </param>
        private void FillFolderTree(string sPath)
        {
            this.FoldersTree.Nodes.Clear();

            DirectoryInfo dirInfo = new DirectoryInfo(sPath);

            TreeNode tnFolder = new TreeNode { Text = dirInfo.Name, Value = dirInfo.FullName };

            this.FoldersTree.Nodes.Add(tnFolder);

            string sFolder = sPath;

            sFolder = sFolder.Substring(this._portalSettings.HomeDirectoryMapPath.Length).Replace("\\", "/");

            ArrayList folders = FileSystemUtils.GetFoldersByParentFolder(this._portalSettings.PortalId, sFolder);
        ...
        /// </param>
        /// <param name="tnColl">
        /// The Nodes we are searching in
        /// </param>
        /// <returns>
        /// The Founded Node
        /// </returns>
        private TreeNode FindNode(string sText, TreeNodeCollection tnColl)
        {
            foreach (TreeNode tnCurr in tnColl)
            {
                if (tnCurr.Text == sText)
                {
                    return tnCurr;
                }

                TreeNode tnFound = this.FindNode(sText, tnCurr.ChildNodes);
        ...
        private PortalSettings GetPortalSettings()
        {
            ...
            try
            {
                ...
                string sPortalAlias = PortalSettings.GetPortalByID(iPortalId, sDomainName);

                PortalAliasInfo objPortalAliasInfo = PortalSettings.GetPortalAliasInfo(sPortalAlias);
        ...
        private void InitializeComponent()
        {
            ...
            this.treeTabs.SelectedNodeChanged += this.TreeTabs_SelectedNodeChanged;
            ...
            this.FoldersTree.SelectedNodeChanged += this.FoldersTree_SelectedNodeChanged;
        }
        ...
        private TreeNode RenderFolder(FolderInfo folderInfo)
        {
            if (
                !FolderPermissionController.HasFolderPermission(
                    this._portalSettings.PortalId, folderInfo.FolderPath, "READ"))
            {
                return null;
            }
        ...
        {
            foreach (TabInfo objTab in this._portalSettings.DesktopTabs)
            {
                // foreach (TabInfo objTab in Globals.GetPortalTabs(_portalSettings.PortalId, false, true, false, false, false))
                if (!objTab.ParentId.Equals(iParentTabId) || !Utility.IsInRoles(objTab.AuthorizedRoles, this._portalSettings) ||
                    objTab.IsDeleted || objTab.StartDate >= DateTime.Now || objTab.EndDate <= DateTime.Now)
                {
                    continue;
                }
                ...
                {
                    this.treeTabs.Nodes.Add(nodeTab);
                }
                ...
                {
                    nodeTab.ImageUrl = this.ResolveUrl("~/images/") + objTab.IconFile;
                }
        ...
        {
            foreach (string language in Localization.GetEnabledLocales())
            {
                var culture = new CultureInfo(language);

                this.LanguageList.Items.Add(new ListItem { Text = culture.DisplayName, Value = language });
            }
        ...
        {
            if (this.treeTabs.Nodes.Count > 0)
            {
                return;
            }
        ...
        private void SetLanguage()
        {
            ...
            this.cmdUploadNow.Text = Localization.GetString("cmdUploadNow.Text", this.ResXFile, this.LanguageCode);
            ...
            {
                case "Flash":
                    allowedExtensionsList = string.Join(",", this.allowedFlashExt);

                    break;
                case "Image":
                    allowedExtensionsList = string.Join(",", this.allowedImageExt);

                    break;
                default:
                    allowedExtensionsList = this.extensionWhiteList;
            ...
            this.cmdDelete.Text = string.Format(
                "<img src=\"Images/DeleteFile.gif\" alt=\"{0}\" title=\"{1}\" />", 
        ...
        private void UploadFile(HttpPostedFile file, string command)
        {
            ...
            if (bAllowUpl)
            {
                ...
                if (File.Exists(sFilePath) && !this.OverrideFile.Checked)
                {
                    ...
                    file.SaveAs(Path.Combine(sUploadDir, fileName));
                }
                else
                {
                    FileSystemUtils.UploadFile(sUploadDir, file);
                }
        ...
        private void CreateFolder_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(this.tbFolderName.Text))
            {
                ...
                finally
                {
                    ...
                    TreeNode tnNewFolder = this.FindNode(this.tbFolderName.Text, this.FoldersTree.Nodes);
                    ...
                    this.Page.ClientScript.RegisterStartupScript(
                        this.GetType(), "ScrollToSelectedNode", "ScrollToSelectedNode();", true);
                }
            }
        ...
        {
            if (this.ctlUpload.PostedFile == null)
            {
                return;
            }

            string fileName = Path.GetFileName(this.ctlUpload.PostedFile.FileName);
            ...
            var fileExtension = Path.GetExtension(this.ctlUpload.PostedFile.FileName);
            ...
            switch (this.browserModus)
            {
                case "Flash":
                    {
                        if (this.allowedFlashExt.Any(sAllowExt => sAllowExt.Equals(fileExtension.ToLower())))
                        {
                            allowUpload = true;
                        }
                    }
                ...
                case "Image":
                    {
                        if (this.allowedImageExt.Any(sAllowExt => sAllowExt.Equals(fileExtension.ToLower())))
                        {
                            allowUpload = true;
                        }
                    }
                ...
                default:
                    {
                        if (this.extensionWhiteList.Contains(fileExtension.ToLower()))
                        {
                            allowUpload = true;
                        }
                    }
            ...
            if (allowUpload)
            {
                ...
                if (File.Exists(sFilePath))
                {
                    ...
                    // FileSystemUtils.UploadFile(lblCurrentDir.Text, ctlUpload.PostedFile, false);
                }
                else
                {
                    // ctlUpload.PostedFile.SaveAs(sFilePath);
            ...
            {
                var error3Script = string.Format(
                    "javascript:alert('{0}')", Localization.GetString("Error3.Text", this.ResXFile, this.LanguageCode));

                this.Page.ClientScript.RegisterStartupScript(this.GetType(), "errorScript", error3Script, true);
            }
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        private void TreeTabs_SelectedNodeChanged(object sender, EventArgs e)
        {
            if (this.treeTabs.SelectedNode == null)
            {
                return;
            ...
            if (this.chkHumanFriendy.Checked)
            {
                ...
                this.rblLinkType.Items[1].Text = Regex.Replace(
                    this.rblLinkType.Items[1].Text,
                    "http://www.MyWebsite.com/Images/MyImage.jpg",
                    Regex.Replace(fileName, domainName, string.Format("{0}", domainName), RegexOptions.IgnoreCase),
                    RegexOptions.IgnoreCase);
            }
            else
            {
        ...
        /// The Event Args e.
        /// </param>
        private void FoldersTree_SelectedNodeChanged(object sender, EventArgs e)
        {
            string sNewDir = this.FoldersTree.SelectedNode.Value;

            this.lblCurrentDir.Text = !sNewDir.EndsWith("\\") ? string.Format("{0}\\", sNewDir) : sNewDir;

            this.ShowFilesIn(sNewDir);
        ...
        private void SetUserFolderPermission(int folderId, UserInfo currentUserInfo)
        {
            ...
            var folderPermissionControler = new FolderPermissionController();

            if (FolderPermissionController.HasFolderPermission(
                this._portalSettings.PortalId, folder.FolderPath, "WRITE"))
            {
                return;
            }

            foreach (
                var folderPermission in
                    from PermissionInfo permission in
                        new PermissionController().GetPermissionsByFolder(this._portalSettings.PortalId, string.Empty)
                    where
                        permission.PermissionKey.ToUpper() == "READ" || permission.PermissionKey.ToUpper() == "WRITE"
            ...
            {
                folderPermissionControler.AddFolderPermission(folderPermission);
            }
        }
