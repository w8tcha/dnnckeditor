    ...
    public partial class CKEditorOptions : PortalModuleBase
    {
        ...
        internal void BindOptionsData(bool reloadOptionsfromModule = false)
        {
            ...
            else
            {
                ...
                {
                    var settingsDictionary = this._portalSettings.HostSettings;
        ...
        protected void Export_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var exportFolderInfo = new FolderController().GetFolderInfo(
                    this._portalSettings.PortalId, Convert.ToInt32(this.ExportDir.SelectedValue));
        ...
        /// <summary>
        /// Delete Settings for the Current Portal or the Current
        /// </summary>
        /// <param name="key">The Portal Or Page Key.</param>
        private void DeletePortalOrPageSettings(string key)
        {
            var hostSettings = new HostSettingsController();

            foreach (
                PropertyInfo info in
                    SettingsUtil.GetEditorConfigProperties())
            {
                hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, info.Name), string.Empty);
            }

            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.SKIN), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CODEMIRRORTHEME), this.CodeMirrorTheme.SelectedValue);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.BROWSER), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.FILELISTPAGESIZE), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.FILELISTVIEWMODE), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.DEFAULTLINKMODE), this.DefaultLinkMode.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.USEANCHORSELECTOR), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.SHOWPAGELINKSTABFIRST), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.SUBDIRS), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.BROWSERROOTDIRID), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.UPLOADDIRID), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.INJECTJS), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.WIDTH), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.HEIGHT), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.BLANKTEXT), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.STYLES), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CSS), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.TEMPLATEFILES), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CONFIG), string.Empty);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.ROLES), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.RESIZEHEIGHT), string.Empty);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.RESIZEWIDTH), string.Empty);

            foreach (RoleInfo objRole in this.objRoleController.GetPortalRoles(this._portalSettings.PortalId))
            {
                hostSettings.UpdateHostSetting(
                    string.Format("{0}{2}#{1}", key, objRole.RoleID, SettingConstants.TOOLB), string.Empty);
            }

            hostSettings.UpdateHostSetting(
                string.Format("{0}{2}#{1}", key, "-1", SettingConstants.TOOLB), string.Empty);
        ...
        private void FillFolders()
        {
            ...
            foreach (FolderInfo folder in FileSystemUtils.GetFolders(this._portalSettings.PortalId))
            {
                string text, value;
                ...
                if (
                    !FolderPermissionController.HasFolderPermission(
                        this._portalSettings.PortalId, folder.FolderPath, "READ"))
                {
                    continue;
                }
        ...
        private void FillInformations()
        {
            ...
            ModuleInfo moduleInfo = new ModuleController().GetModuleByDefinition(
                this._portalSettings.PortalId, "User Accounts");

            this.iModuleId = this.ModuleId == -1 ? moduleInfo.ModuleID : this.ModuleId;

            try
            {
                moduleDefinitionInfo =
                    moduleDefinitionController.GetModuleDefinition(this.ModuleConfiguration.ModuleDefID);
            }
            catch (Exception)
            {
                moduleDefinitionInfo = moduleDefinitionController.GetModuleDefinitionByName(
                    moduleInfo.DesktopModuleID, "User Accounts");
            }

            var type = typeof(Web.CKEditorControl);
            var assembly = new AssemblyName(Assembly.GetAssembly(type).FullName);

            this.ProviderVersion.Text += assembly.Version.ToString();
        ...
        private void FillRoles()
        {
            ...
            foreach (RoleInfo objRole in this.objRoleController.GetPortalRoles(this._portalSettings.PortalId))
            {
                ...
                if (objRole.RoleName.Equals(this._portalSettings.AdministratorRoleName))
                {
        ...
        private void FillSkinList()
        {
            ...
            var themesFolder =
                new DirectoryInfo(this.MapPath(this.ResolveUrl("~/Providers/HtmlEditorProviders/CKEditor/plugins/codemirror/theme")));
        ...
        private PortalSettings GetPortalSettings()
        {
            ...
            try
            {
                ...
                if (this.request.QueryString["tid"] != null)
                {
                ...
                var portalAlias = PortalSettings.GetPortalByID(this.CurrentOrSelectedPortalId, domainName);

                portalSettings = new PortalSettings(this.CurrentOrSelectedTabId, PortalSettings.GetPortalAliasInfo(portalAlias));
            }
            catch (Exception)
            {
        ...
        private void IbDeleteClick(object sender, ImageClickEventArgs e)
        {
            ...
            this.listToolbars.RemoveAll(
                toolbarSel => toolbarSel.Name.Equals(this.dDlCustomToolbars.SelectedValue));
        ...
        private void InitializeComponent()
        {
            ...
            this.listToolbars = ToolbarUtil.GetToolbars(this._portalSettings.HomeDirectoryMapPath, this.configFolder);
        ...
        private void ToolbarGroupsRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            foreach (var button in this.toolbarSets.ToolbarGroups.Where(@group => @group.name.Equals(groupName)).SelectMany(@group => @group.items))
            {
                if (this.listButtons == null)
                {
                    this.listButtons = ToolbarUtil.LoadToolBarButtons(
                            !string.IsNullOrEmpty(this.configFolder)
                                ? Path.Combine(this._portalSettings.HomeDirectoryMapPath, this.configFolder)
                                : this._portalSettings.HomeDirectoryMapPath);
                }
        ...
        private void LoadSettings(int currentMode, bool changeMode = true)
        {
            ...
            this.lnkRemoveAll.Visible = false;
            this.lnkRemoveChild.Visible = false;
        ...
        private string ReFormatURL(string inputUrl)
        {
            ...
            return string.Format(
                "FileID={0}", objFileController.ConvertFilePathToFileId(inputUrl, this._portalSettings.PortalId));
        }
        ...
        private void Remove_Click(object sender, EventArgs e)
        {
            ...
            {
                case SettingsMode.Portal:
                    this.DeletePortalOrPageSettings(string.Format("DNNCKP#{0}#", this._portalSettings.PortalId));
                    break;
                case SettingsMode.Page:
                    this.DeletePortalOrPageSettings(string.Format("DNNCKT#{0}#", this.CurrentOrSelectedTabId));
                    break;
                case SettingsMode.ModuleInstance:
        ...
        private void RemoveAll_Click(object sender, EventArgs e)
        {
            switch (this.CurrentSettingsMode)
            {
             ...
             this.ShowNotification(Localization.GetString("lblInfoDel.Text", this.ResXFile, this.LangCode),
                    "success");
        }
        ...
        private void SaveModuleSettings()
        {
            ...
            {
                moduleController.UpdateModuleSetting(
                    this.iModuleId, string.Format("{0}{1}", key, SettingConstants.WIDTH), this.txtWidth.Text);
            }

            if (Utility.IsUnit(this.txtHeight.Text))
            {
                moduleController.UpdateModuleSetting(
                    this.iModuleId, string.Format("{0}{1}", key, SettingConstants.HEIGHT), this.txtWidth.Text);
            }
        ...
        private void SavePortalOrPageSettings(string key)
        {
                    ...
                    case "String":
                        {
                            ...
                            if (textBox != null)
                            {
                                hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, info.Name), textBox.Text);
                            }
                        }
                    ...
                    case "Boolean":
                        {
                            ...
                            if (checkBox != null)
                            {
                                hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, info.Name), checkBox.Checked.ToString());
                            }
                        }
                    ...
                    case "DefaultLinkType":
                        {
                            ...
                            if (dropDownList != null)
                            {
                                if (dropDownList.SelectedItem != null)
                                {
                                    hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, info.Name), dropDownList.SelectedValue);
                                }
                            }
                        }
                                    ...
                                    case "String":
                                        {
                                            ...
                                            {
                                                hostSettings.UpdateHostSetting(
                                                    string.Format("{0}{1}", key, codeMirrorInfo.Name), textBox.Text);
                                            }
                                        }
                                    ...
                                    case "String":
                                        {
                                            ...
                                            {
                                                hostSettings.UpdateHostSetting(
                                                    string.Format("{0}{1}", key, wordCountInfo.Name), textBox.Text);
                                            }
                                        }
            ...
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.SKIN), this.ddlSkin.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.CODEMIRRORTHEME), this.CodeMirrorTheme.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.BROWSER), this.ddlBrowser.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.FILELISTVIEWMODE), this.FileListViewMode.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.DEFAULTLINKMODE), this.DefaultLinkMode.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.USEANCHORSELECTOR),
            ...
            hostSettings.UpdateHostSetting(
                ...
                this.ShowPageLinksTabFirst.Checked.ToString());
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.SUBDIRS), this.cbBrowserDirs.Checked.ToString());
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.BROWSERROOTDIRID), this.BrowserRootDir.SelectedValue);
            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.UPLOADDIRID), this.UploadDir.SelectedValue);

            if (Utility.IsNumeric(this.FileListPageSize.Text))
            {
                hostSettings.UpdateHostSetting(
                    string.Format("{0}{1}", key, SettingConstants.FILELISTPAGESIZE), this.FileListPageSize.Text);
            }

            if (Utility.IsNumeric(this.txtResizeWidth.Text))
            {
                hostSettings.UpdateHostSetting(
                    string.Format("{0}{1}", key, SettingConstants.RESIZEWIDTH), this.txtResizeWidth.Text);
            }

            if (Utility.IsNumeric(this.txtResizeHeight.Text))
            {
                hostSettings.UpdateHostSetting(
                    string.Format("{0}{1}", key, SettingConstants.RESIZEHEIGHT), this.txtResizeHeight.Text);
            }

            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.INJECTJS), this.InjectSyntaxJs.Checked.ToString());

            if (Utility.IsUnit(this.txtWidth.Text))
            {
                hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.WIDTH), this.txtWidth.Text);
            }

            if (Utility.IsUnit(this.txtHeight.Text))
            {
                hostSettings.UpdateHostSetting(
                    string.Format("{0}{1}", key, SettingConstants.HEIGHT), this.txtHeight.Text);
            }

            hostSettings.UpdateHostSetting(
                string.Format("{0}{1}", key, SettingConstants.BLANKTEXT), this.txtBlanktext.Text);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.STYLES), this.StylesURL.Url);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CSS), this.CssUrl.Url);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.TEMPLATEFILES), this.TemplUrl.Url);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CUSTOMJSFILE), this.CustomJsFile.Url);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CUSTOMJSFILE), this.CustomJsFile.Url);
            hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.CONFIG), this.ConfigUrl.Url);
            ...
            if (sRoles != string.Empty)
            {
                hostSettings.UpdateHostSetting(string.Format("{0}{1}", key, SettingConstants.ROLES), sRoles);
            }
            ...
            for (int i = 0; i < this.gvToolbars.Rows.Count; i++)
            {
                ...
                {
                    hostSettings.UpdateHostSetting(
                        string.Format("{0}{2}#{1}", key, "-1", SettingConstants.TOOLB), ddLToolB.SelectedValue);
                }
                else
                {
                    RoleInfo objRole = this.objRoleController.GetRoleByName(this._portalSettings.PortalId, label.Text);
                    ...
                    hostSettings.UpdateHostSetting(
                        string.Format("{0}{2}#{1}", key, objRole.RoleID, SettingConstants.TOOLB), ddLToolB.SelectedValue);
                }
            }
        ...
        {
            ModuleDefinitionController dbm = new ModuleDefinitionController();
            ...
            {
                objm = dbm.GetModuleDefinitionByName(moduleInfo.DesktopModuleID, "User Accounts");
            }
        ...
        private void SetLanguage()
        {
            ...
            this.lnkRemove.Text = Localization.GetString("lnkRemove.Text", this.ResXFile, this.LangCode);
